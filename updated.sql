...
(12, NULL, 'Automatic', 'Near Entrance', NULL),
(13, NULL, 'Regular', 'Truck Parking', NULL),
(14, NULL, 'Compact', 'Two-Wheeler', NULL),
(15, NULL, 'Electric', 'In the Middle', NULL);
...
(12, 'Full Day', 35, 'Fixed rate for full day'),
(13, 'Weekly', 50, 'Subscription for a week'),
(14, 'Monthly', 200, 'Subscription for a month'),
(15, 'Daily', 10, 'Subscription for a day');
...
-- Enhanced procedure with parameters
DELIMITER //
CREATE PROCEDURE GetAvailableSlots11(
    IN p_slot_type VARCHAR(50),        -- Input parameter: filter by slot type
    IN p_max_fee DECIMAL(10,2),       -- Input parameter: maximum fee
    OUT p_total_available INT          -- Output parameter: total available slots
)
BEGIN
    ...
    JOIN Parking_Fees pf ON ps.parking_slot_type = pf.parking_fees_type
    WHERE ps.customer_id IS NULL
    AND (p_slot_type IS NULL OR ps.parking_slot_type = p_slot_type)
    AND (p_max_fee IS NULL OR pf.parking_fees_amount <= p_max_fee);
    
    -- Set output parameter with count of available slots
    SELECT COUNT(*) INTO p_total_available
    FROM Parking_Slots ps
    JOIN Parking_Fees pf ON ps.parking_slot_type = pf.parking_fees_type
    WHERE ps.customer_id IS NULL
    AND (p_slot_type IS NULL OR ps.parking_slot_type = p_slot_type)
...
CALL GetAvailableSlots11(NULL, NULL, @total);
SELECT @total AS 'Total Available Slots';

-- 2. Get only compact slots
CALL GetAvailableSlots11('Compact', NULL, @total);
SELECT @total AS 'Total Compact Slots Available';

-- 3. Get slots with fees <= 10.00
CALL GetAvailableSlots11(NULL, 10.00, @total);
SELECT @total AS 'Total Slots Under $10';

-- 4. Get compact slots with fees <= 8.00
CALL GetAvailableSlots11('Compact', 8.00, @total);
SELECT @total AS 'Total Compact Slots Under $8';
